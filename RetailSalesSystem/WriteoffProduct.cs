...
{
    public partial class WriteoffProduct : Form
    {
        ...
        {
            string selectCommand = "select Номенклатор, sum(Количество) as Количество, sum(`Сумма оптовая`) as `Суммарная оптовая цена`, sum(`Сумма розничная`) as `Суммарная розничная цена` from `write-off product` where Дата >= '" + Program.data1.ToString("yyyy-MM-dd") + "' and Дата <= '" + Program.data2.ToString("yyyy-MM-dd") + "'group by Номенклатор;";
            try
            {
