...
{
    public partial class СonsumptionProduct : Form
    {
        MySqlDataAdapter dataAdapter;
        MySqlConnection connection;
        MySqlCommandBuilder commandBuilder;
        BindingSource binding;
        public СonsumptionProduct()
        {
            InitializeComponent();
            GetData();
        }
        ...
        {
            string selectCommand = "select Номенклатор, sum(Количество) as Количество, sum(`Сумма оптовая`) as `Суммарная оптовая цена`, sum(`Сумма розничная`) as `Суммарная розничная цена` from `consumption product` where Дата >= '" + Program.data1.ToString("yyyy-MM-dd") + "' and Дата <= '" + Program.data2.ToString("yyyy-MM-dd") + "'group by Номенклатор;";
            try
            {
