...
{
    public partial class RDDepartmen : Form
    {
        ...
        {
            string selectCommand = "select контрагент.Псевдоним,контрагент.`Полное название`, контрагент.Отдел,`заголовки накладной`.Номер,`заголовки накладной`.Контрагент,`заголовки накладной`.Дата,`табличная часть накладной`.`Номер ЗН`,`табличная часть накладной`.Количество, `табличная часть накладной`.Номенклатор from контрагент,`заголовки накладной`,`табличная часть накладной` where контрагент.Псевдоним = `заголовки накладной`.Контрагент and `заголовки накладной`.Номер = `табличная часть накладной`.`Номер ЗН` and `заголовки накладной`.Дата >= '" + Program.data1.ToString("yyyy-MM-dd") + "' and `заголовки накладной`.Дата <= '" + Program.data2.ToString("yyyy-MM-dd") + "';";
            try
            {
                ...
                using (connection = new MySqlConnection(Program.connectionString))
                {
                    ...
                    table.Columns.Remove("Псевдоним");
                    table.Columns.Remove("Полное название");
                    table.Columns.Remove("Номер");
                    table.Columns.Remove("Контрагент");
                    table.Columns.Remove("Номер ЗН");
